...
export const pickYoutubeVideoInformation = (): youtubeVideoInformationInterface => {
    const randomNumber = Math.floor(Math.random() * (youtubeVideoInformation.length))
...
export const youtubeViewerOnReady = (startTime: number) => (event: YouTubeEvent) => {
